...
export class DadosConfService {
  ...
  async create(createDadosConfDto: CreateDadosConfDto): Promise<DadosConf> {
    const dadosConf = dadosConfRepository.create({
      ...
      itens: createDadosConfDto.itens.map((itemDto) =>
        dadosConfItemRepository.create(itemDto)
      ),
    });
  ...
  async update(
    id: number,
    updateDadosConfDto: UpdateDadosConfDto
  ): Promise<DadosConf | null> {
  ...
  async findItensByDadosConfId(idDadosConf: number): Promise<DadosConfItem[]> {
    return dadosConfItemRepository.find({
      where: { idDadosConf: idDadosConf },
    });
  }
  ...
  async findOneItem(
    idDadosConf: number,
    itemId: number
  ): Promise<DadosConfItem | null> {
    return dadosConfItemRepository.findOne({
      where: { idDadosConf: idDadosConf, id: itemId },
    });
  }

  async createItem(
    idDadosConf: number,
    createDadosConfItemDto: CreateDadosConfItemDto
  ): Promise<DadosConfItem | null> {
    const dadosConf = await dadosConfRepository.findOneBy({ id: idDadosConf });
    if (!dadosConf) {
      return null;
    }
    const newItem = dadosConfItemRepository.create({
  ...
  async updateItem(
    idDadosConf: number,
    itemId: number,
    updateDadosConfItemDto: UpdateDadosConfItemDto
  ): Promise<DadosConfItem | null> {
    const item = await dadosConfItemRepository.findOne({
      where: { idDadosConf: idDadosConf, id: itemId },
    });
    if (!item) {
      return null;
  ...
  async deleteItem(idDadosConf: number, itemId: number): Promise<boolean> {
    const result = await dadosConfItemRepository.delete({
      idDadosConf: idDadosConf,
      id: itemId,
    });
    return (result.affected ?? 0) > 0;
  }
